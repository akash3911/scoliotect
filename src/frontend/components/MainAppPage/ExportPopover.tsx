...
import ScoliotectDocument from "./ScoliotectDocument";
...
const ExportPopover: React.FC<IExportPopoverProps> = ({}) => {
  ...
  function handleDownloadImage(imageType: "png" | "jpeg") {
    ...
    return () => {
      ...
      imageLink.download = `ScoliotectResult_${monthDay}_${time}.${imageType}`;
            ...
            {pdfDownloadIsAvailable ? (
              <BlobProvider
                document={
                  <ScoliotectDocument
                    imageSrc={canvasURL}
                    scoliotectAPIResponse={scoliotectAPIResponse}
                  />
                }
              >
