...
function ColorSwatchControl({
  ...
  return (
    <div className="grid grid-cols-[50px,1fr] items-center border border-transparent pl-3 h-10 gap-x-2">
      ...
      <div className="flex items-center gap-2">
        {COLOR_PRESETS.map((preset) => (
          <button
            ...
            className={`h-7 w-7 rounded-full border-2 shadow-sm transition transform hover:scale-105 ${
        ...
        <label className="relative h-7 w-7 cursor-pointer overflow-hidden rounded-full border border-gray-300 shadow-sm bg-white">
          <span
...
const MainAppPage = () => {
  ...
  const setScoliotectAPIResponse = useStore(
    (state) => state.setScoliotectAPIResponse
  );
  ...
  const setDetectionColor = useStore((state) => state.setDetectionColor);
  const setDetectionTextSize = useStore((state) => state.setDetectionTextSize);
  const setShowDetectionLabels = useStore(
    (state) => state.setShowDetectionLabels
  );
  const setLineColor = useStore((state) => state.setLineColor);
  const setLineScale = useStore((state) => state.setLineScale);
  const setCobbLineScale = useStore((state) => state.setCobbLineScale);
  const setCobbTextSize = useStore((state) => state.setCobbTextSize);
  ...
  async function fetchData(file: ISelectedFile) {
    ...
    try {
      ...
      setScoliotectAPIResponse(res.data);
  ...
  useEffect(() => {
    return () => {
      // Reset page-scoped state so reopening /app starts with a clean slate.
      setSelectedFile(undefined);
      setScoliotectAPIResponse(undefined);
    };
  }, [setSelectedFile, setScoliotectAPIResponse]);
      ...
      >
        ...
        <SideBarContainer>
          <h1 className="text-lg text-primary text-center">
            <span className="font-black"></span>Scoliosis Xray detection
          </h1>
          <hr />
            ...
            <>
              ...
              {drawSettings.showDetections && (
                <div className="flex flex-col gap-y-0.5">
                  ...
                  <ColorSwatchControl
                    label="Color"
                    color={drawSettings.detectionColor}
                    onChange={setDetectionColor}
                  />
                  <div className="grid grid-cols-[50px,1fr] items-center border border-transparent pl-3 h-10 gap-x-2">
                    <label className="text-xs truncate">Text</label>
                    <div className="h-7 select-none flex">
                      <input
                        className="cursor-grab active:cursor-grabbing"
                        type="range"
                        ...
                        step="0.25"
                        onChange={(e) => {
                          setDetectionTextSize(parseFloat(e.target.value));
                        }}
                        value={drawSettings.detectionTextSize}
                      />
                    </div>
                  </div>
                </div>
              ...
              {drawSettings.showLandmarks && (
                <div className="flex flex-col gap-y-0.5">
                  ...
                  <ColorSwatchControl
                    ...
                    onChange={(color) =>
                      setLandmarkColor({ topColor: color })
                    }
                  />
                  <ColorSwatchControl
                    label="Bottom"
                    ...
                    onChange={(color) =>
                      setLandmarkColor({ bottomColor: color })
                    }
                  />
                  <ColorSwatchControl
                    label="Join"
                    color={drawSettings.lineColor}
                  ...
                  <div className="grid grid-cols-[50px,1fr] items-center border border-transparent pl-3 h-10 gap-x-2">
                    ...
                    <div className="h-7 select-none flex">
                      <input
                        ...
                        step="0.05"
                        onChange={(e) => {
                          setLineScale(parseFloat(e.target.value));
                        }}
                        value={drawSettings.lineScale}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-[50px,1fr] items-center border border-transparent pl-3 h-10 gap-x-2">
                    ...
                    <div className="h-7 select-none flex">
                      <input
                        ...
                        step="0.1"
                        onChange={(e) => {
                          setCobbLineScale(parseFloat(e.target.value));
                        }}
                        value={drawSettings.cobbLineScale}
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-[50px,1fr] items-center border border-transparent pl-3 h-10 gap-x-2">
                    ...
                    <div className="h-7 select-none flex">
                      <input
                        ...
                        step="1"
                        onChange={(e) => {
                          setCobbTextSize(parseInt(e.target.value));
                        }}
                        value={drawSettings.cobbTextSize}
                      />
                    </div>
                  </div>
                  <p className="text-[11px] text-gray-500 pl-3">
                    Drag Cobb labels directly on the image to reposition them.
                  </p>
                </div>
              )}
              <hr />
              {scoliotectAPIResponse && scoliotectAPIResponse.angles ? (
                <div className="flex flex-col gap-y-3 text-sm text-gray-700">
                  <p className="font-semibold">Results</p>
                  <div>Curve Type: {scoliotectAPIResponse.curve_type}</div>
                  <div>
                    <div className="grid grid-cols-[210px,1fr]">
                      <span>Proximal Thoracic (PT):</span>
                      <span>
                        {scoliotectAPIResponse.angles.pt.angle.toFixed(2)}°
                      </span>
                    </div>
                    <div className="grid grid-cols-[210px,1fr]">
                      <span>Main Thoracic (MT):</span>
                      <span>
                        {scoliotectAPIResponse.angles.mt.angle.toFixed(2)}°
                      </span>
                    </div>
                    <div className="grid grid-cols-[210px,1fr]">
                      <span>Thoracolumbar/Lumbar (TL/L):</span>
                      <span>
                        {scoliotectAPIResponse.angles.tl.angle.toFixed(2)}°
                      </span>
                    </div>
                  </div>
                  <p>
                    ...
                    <b>
                      (
                      {scoliotectAPIResponse.angles[
                        getMaxCobbAngle(scoliotectAPIResponse.angles).max
                      ].idxs[0] + 1}
                      )
                    </b>{" "}
          ...
          {!scoliotectAPIResponse && !selectedFile && !loading && (
  ...
  return (
    <div className="flex flex-col justify-center h-full gap-y-5">
      ...
      <p className="text-center text-sm text-gray-600 px-4">
         is currently sending your spine to the server. Please wait a
        while...
      </p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col justify-center flex-1 gap-y-5">
      ...
      <div className="flex flex-col gap-y-2 max-w-xs w-full mx-auto">
        ...
        <p className="text-center text-sm text-gray-600 px-4">
          You haven&apos;t chosen an image yet. Upload or choose an image to get started!
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col justify-center flex-1 gap-y-5">
      ...
      <div className="flex flex-col gap-y-2 max-w-xs w-full mx-auto">
        ...
        <p className="text-center text-sm text-gray-600 px-4">
          The server did not respond so the request failed.
        </p>
      </div>
      <button
